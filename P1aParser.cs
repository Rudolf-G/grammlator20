...
namespace grammlator {
   ...
   internal sealed partial class P1aParser : GrammlatorApplication {
      ...
      private void SetGrammlatorStringSetting(Int32 nameIndex, Int32 stringIndex)
      {
         ...
         SetGrammlatorStringSetting(nameIndex, NewValue);
      }

      private void SetGrammlatorStringSetting(Int32 nameIndex, String assignedString)
      {
         ...
         case Int64Setting i32s:
            if (Int32.TryParse(assignedString, out Int32 newIntValue))
      ...
      //|   | Name(Int32 nameIndex), ":", Number(Int64 value), ";"?
      private void SetGrammlatorInt32Setting(Int32 nameIndex, Int64 value)
      {
         String Name = GlobalVariables.GetStringOfIndex(nameIndex);
         String NameToLower = Name.ToLower();
         Setting? s = GlobalVariables.VisibleSettings.Find(s => s.NameToLower == NameToLower);
      ...
      //|   | Name(Int32 nameIndex), ":", Name(Int32 valueIndex), ";"?
      private void SetGrammlatorNameSetting(Int32 nameIndex, Int32 valueIndex)
         => SetGrammlatorStringSetting(nameIndex, GlobalVariables.GetStringOfIndex(valueIndex));
      ...
      private void CompareTerminalDeclarationsWithEnum()
      {
         ...
         if (GlobalVariables.TerminalSymbolEnum.Value == "")
            GlobalVariables.TerminalSymbolEnum.Value = GlobalVariables.GetStringOfIndex(EnumNameStringIndex);

         AddMissingTerminalsFromEnum();
      ...
      //|  TerminalSymbol=
      //|      "Name(Attributes)"(Int32 nameIndex, Int32 NumberOfAttributes), OptionalWeight(Int64 Weight)
      private void TerminalSymbol(Int32 nameIndex, Int32 NumberOfAttributes, Int64 Weight)
      ...
      //| CSEnumMember=
      //|    OptionalDescriptionAttribute(String description), Name(Int32 enumElementStringIndex), OptionalEnumElementNumber(Int64 enumNumber)
      private void EnumElementRecognized(String description, Int32 enumElementStringIndex, Int64 enumNumber)
      {
         ...
         if (description != "")
         {
            ...
            if (description.Length < 2 || !(description[^1] == '"')) // "" is used as end marker => do not need to check Position>=description.length
               goto DescriptionError;
            SkipWhiteSpace(description, ref Position);
            ...
            while (description[Position] != ')')
            {
               ...
               Is1st = false;
               if (!IsIdentifier(description, ref Position, out string ArgumentType))
                  goto DescriptionError;
               SkipWhiteSpace(description, ref Position);
               if (!IsIdentifier(description, ref Position, out String ArgumentName))
                  goto DescriptionError;
 ...
 @$"Error in enum: the terminal symbol {Name} has already been defined with a different argumenttype {TypeIofT} / {ArgumentTypes[i]} ");
      ...
      Boolean IsIdentifier(String s, ref int position, out String identifier)
      {
         identifier = "";
         int StartPosition = position;
         if (!char.IsLetter(s[position]))
            return false;
         position++;
         while (char.IsLetter(s[position]) || char.IsDigit(s[position]))
            position++;
         identifier = s.Substring(StartPosition, position - StartPosition);
         return true;
      }

      void SkipWhiteSpace(String s, ref int Position)
      {
         while (char.IsWhiteSpace(s[Position]))
            Position++;
         return;
      }
      ...
      //|    | "[", Name(Int32 attributeIdentifierStringIndex) , "(", LexerString(Int32 descriptionStringIndex), ")", "]"
      private void DescriptionAttribute(out String description, Int32 attributeIdentifierStringIndex, Int32 descriptionStringIndex)
      {
         String AttributeIdentifier = GlobalVariables.GetStringOfIndex(attributeIdentifierStringIndex);
         if (AttributeIdentifier != "Description")
            P1OutputMessageAndLexerPosition(MessageTypeOrDestinationEnum.Error,
               @$"Expected attribute ""Description"", found ""{AttributeIdentifier}""");
         description = GlobalVariables.GetStringOfIndex(descriptionStringIndex);
      }
      //|
      //|   /* end of grammar */
      #endregion grammar
      ...
      State84:
         const String StateDescription84 =
              "CSEnumMember= OptionalDescriptionAttribute(String description), ►Name(Int32 enumElementStringIndex), OptionalEnumElementNumber(Int64 enumNumber);";
         ParserInput = Lexer.PeekSymbol();
         if (ParserInput != LexerResult.Name)
         {
      ...
      State3:
         const String StateDescription3 =
              "TerminalSymbol= \"Name(Attributes)\"(Int32 nameIndex, Int32 NumberOfAttributes), ►OptionalWeight(Int64 Weight);";
         ParserInput = Lexer.PeekSymbol();
      ...
      State74:
         const String StateDescription74 =
            ...
            + "GrammlatorSetting= Name(Int32 nameIndex), ►\":\", Number(Int64 value), \";\"?;\r\n"
      ...
      State87:
         const String StateDescription87 =
              "OptionalDescriptionAttribute(String description)= \"[\", ►Name(Int32 attributeIdentifierStringIndex), \"(\", LexerString(Int32 descriptionStringIndex), \")\", \"]\";";
         ParserInput = Lexer.PeekSymbol();
         if (ParserInput != LexerResult.Name)
         {
      ...
      State91:
         ...
         DescriptionAttribute(
            ...
            attributeIdentifierStringIndex: _a.PeekClear(-1)._Int32,
            descriptionStringIndex: _a.PeekRef(0)._Int32
            );
