...
namespace Grammlator
{
    ...
    internal sealed class ParserState : ParserAction, IELementOfPartition
    {
        ...
        public void FindAndResolveStaticConflictsOfState(StringBuilder sb, BitArray allowedSymbolsUpToThisAction, BitArray dynamicSymbols)
        {
            ...
            var State = this;
            // Alle Aktionen suchen, die mindestens ein
            // mit einer der vorhergehenden Aktionen
            // gemeinsames Aktionssymbol haben (=Konflikt)
            ...
            for (Int32 IndexOfAction = 0; IndexOfAction < State.Actions.Count; IndexOfAction++)
            {
                ...
                switch (State.Actions[IndexOfAction])
                {
                ...
                {
                    writeStateHeader =
                    State.SolveConflicts(sb, writeStateHeader, IndexOfAction, symbolsOfThisAction, allowedSymbolsUpToThisAction);
