...
namespace Grammlator
{
   ...
   internal abstract class ParserAction : IComparable<ParserAction>, IUniqueIndex, IParserAction
   {
      ...
      public Int32 Codenumber {
         get; set;
      } // TODO use Codenumber in Phase 5
   ...
   internal sealed class Definition : ParserAction
   {
      ...
      public override Int32 CompareTo(ParserAction other)
      {
         ...
         // Ergebnis <0, wenn "this ist vor v" zutrifft
         // Sortieren nach Aktionsart
         if (this.ParserActionType != other.ParserActionType)
            return (Int32)this.ParserActionType - (Int32)other.ParserActionType;

         var vAlternative = (Definition)other;
         return this.DefinedSymbol.SymbolNumber != vAlternative.DefinedSymbol.SymbolNumber
             ? this.DefinedSymbol.SymbolNumber - vAlternative.DefinedSymbol.SymbolNumber
      ...
      // TODO Check: Must dynamic priority be considered here or has it been evaluated in phase 4 ??
      ...
      internal EmptyComputationResultEnum IsEmptyAlternativeComputation()
      {
   ...
   internal class ListOfDefinitions : List<Definition>
   {
      ...
      /// <summary>
      /// Search all alternatives of the list for an empty one
      /// </summary>
      private EmptyComputationResultEnum _EmptyComputationResult;

      internal EmptyComputationResultEnum ListContainsEmptyAlternativeComputation()
      {
         switch (_EmptyComputationResult)
         {
         ...
         // Redundant check: is any of the alternatives empty
         ...
         // Erzeugt eine der Alternativen (mittelbar) die leere Zeichenkette?
         _EmptyComputationResult = EmptyComputationResultEnum.IsJustBeingComputed;
         Boolean SearchLimitedByRecursion = false;

         foreach (Definition Alternative in this)
         {
            switch (Alternative.IsEmptyAlternativeComputation())
            {
         ...
         // did not find an empty alternative
   ...
   internal abstract class ParserActionWithNextAction : ParserAction
   {
      ...
      public override Int32 CompareTo(ParserAction other)
      { // Prüfen: wird in Phase 2 vewendet, in Phase 5 gilt ein andres CompareTo
         if (other == null)
            throw new ArgumentNullException($"CompareTo({nameof(other)})");

         if (this.ParserActionType != other.ParserActionType)
            return (Int32)this.ParserActionType - (Int32)other.ParserActionType;
         //if (this.Nummer != v.Nummer)
         //    return this.Nummer - v.Nummer;
         // weitere Kriterien?

         return this.IdNumber - other.IdNumber;

         // TODO bisherige Sortierfolge verbessern? siehe nachfolgenden Ansatz ?
         //// v ist cAktionMitFolgeaktion:
         ////   die jeweiligen Foleaktionen vergleichen.
         //// TODO Achtung: Gefahr unendlicher Rekursion
         //return Folgeaktion.CompareTo(((cAktionMitFolgeaktion)v).Folgeaktion);
      }
...
#pragma warning restore CA1812 // Avoid uninstantiated internal classes
   ...
   { // TODO Check ": ActionWithNextAction" ??? no, NextAction should be ignored because it can not be reached
      public ParserAction NextAction {
         get;
      }
