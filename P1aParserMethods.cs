...
namespace Grammlator
    ...
    internal partial class P1aParser
        ...
        /// <summary>
        /// The name of the last enum the parser found in the source.
        /// Is null if an optional enum has been empty.
        /// </summary>
        ...
        private void TerminalSymbolDeclaration(String Name, Int32 numberOfAttributes, Int32 weight)
            {
            Debug.Assert(!String.IsNullOrEmpty(Name), $"{nameof(Name)} IsNullOrEmpty");
            ...
            if (SymbolDictionary.ContainsKey(Name))
                ...
                SymbolDictionary[Name] =
                   new TerminalSymbol(Name) {
                       ...
                       AttributetypeList = ListOfAttributesOfGrammarRule.GetAttributeTypes(numberOfAttributes),
                       AttributenameList = ListOfAttributesOfGrammarRule.GetAttributeIdentifiers(numberOfAttributes)
                       };
                }
        ...
        private NonterminalSymbol NonterminalSymbolDefinition(String symbolName, Int32 numberOfAttributes)
            {
            ...
            if (!SymbolDictionary.TryGetValue(symbolName, out Symbol Symbol))
                ...
                Debug.Assert(ns.AttributenameList == null || ns.AttributenameList.Length == 0);
                ns.AttributenameList = ListOfAttributesOfGrammarRule.GetAttributeIdentifiers(numberOfAttributes);
                }
            ...
            for (Int32 i = 1; i <= numberOfAttributes; i++)
                {
                AttributeStruct a = ListOfAttributesOfGrammarRule[ListOfAttributesOfGrammarRule.Count - i];
                a.LeftSide = true;
                a.OverlayType = AttributeStruct.OverlayEnum.outAttribute;
                ListOfAttributesOfGrammarRule[ListOfAttributesOfGrammarRule.Count - i] = a;
                }
        ...
        private Symbol EvaluateSymbolnameFoundInRightSide(String Name, Int32 NumberOfAttributes)
            {
            if (SymbolDictionary.TryGetValue(Name, out Symbol symbol) /*Symbol == null*/)
                ...
                symbol = new NonterminalSymbol(Name) { SymbolNumber = SymbolDictionary.Count - GlobalVariables.NumberOfTerminalSymbols };
                ...
                symbol.AttributetypeList
                    = NumberOfAttributes == 0
                    ? emptyListOfStrings
                    : ListOfAttributesOfGrammarRule.GetAttributeTypes(NumberOfAttributes);
                }
            return symbol;
            }
        ...
        private void UseNonterminalInRightSide(Symbol symbol)
            ...
            for (Int32 i = 1; i <= symbol.NumberOfAttributes; i++)
                ...
                AttributeStruct Attribut = ListOfAttributesOfGrammarRule[ListOfAttributesOfGrammarRule.Count - i];
                Attribut.Level--;
                Attribut.LeftSide = false;
                Attribut.OverlayType = AttributeStruct.OverlayEnum.inAttribute;
                ListOfAttributesOfGrammarRule[ListOfAttributesOfGrammarRule.Count - i] = Attribut;
                }
            }
        ...
        private String MakeNewName(String prefix) => $"({prefix}{(++CountOfGeneratedNames).ToString()})";
        ...
        private static String MakeNewName(TypeOfGrammarRule type, string nameOfSymbol)
            ...
            switch (type)
                ...
                case TypeOfGrammarRule.repeat1lr:
                    Postfix = "+";
                    break;
                case TypeOfGrammarRule.repeat1rr:
                    Postfix = "++";
                    break;
                default:
                    Postfix = "-??";
        ...
        private NonterminalSymbol DefineNewSymbol(Symbol existingSymbol, TypeOfGrammarRule type, String NewName)
            ...
            var newSymbol = new NonterminalSymbol(NewName) {
                SymbolNumber = SymbolDictionary.Count - GlobalVariables.NumberOfTerminalSymbols,
                AttributetypeList = ListOfAttributesOfGrammarRule.GetAttributeTypes(0), // has no attributes
                AttributenameList = ListOfAttributesOfGrammarRule.GetAttributeIdentifiers(0),
        ...
        private void PushAttributeToListOfAttributesOfGrammarRule(String NewType, String NewName)
            {
            if (String.IsNullOrEmpty(NewName))
                throw new ErrorInGrammlatorProgramException("Attribute without name");
            ...
            if (overlayedAttribute.Name == NewName)
                {
                ...
                if (overlayedAttribute.PositionInProduction != AttributeCounter)
                    ...
                    P1OutputMessageAndLexerPosition(MessageTypeOrDestinationEnum.Error,
                        $"The defined nonterminal or the definition already contains an attribute \"{NewName}\", but with different position (does not overlay)."
                        );
                    }
                else if (overlayedAttribute.Type != NewType)
                    {// overlaying attributes must have the same type
                    P1OutputMessageAndLexerPosition(MessageTypeOrDestinationEnum.Error,
                        $"The defined nonterminal or the definition already contains an overlaying attribute \"{NewName}\", but with different type."
                        );
                    }
                else
                ...
                if (overlayedAttribute.Type != newAttribute.Type)
                    {
        ...
        private readonly String[] emptyListOfStrings = Array.Empty<String>();
        ...
        private Boolean AttributeTypesCoincide(Symbol symbol)
            ...
            for (Int32 i = 0; i < NumberOfAttributes; i++)
                {
                if (symbol.AttributetypeList[i]
                    != ListOfAttributesOfGrammarRule[ListOfAttributesOfGrammarRule.Count - NumberOfAttributes + i].Type)
                    {
                    return false;
                    }
                }
        ...
        private void MethodProperties(out MethodClass method, String methodModifier, String type, String name)
            {
        ...
        /// <param name="type">must bei "void" else NotImplementedException</param>
        /// <param name="name">Name of the method</param>
        private void MethodProperties(out MethodClass method, String methodModifier1, String methodModifier2, String type, String name)
            {
            if (!CSharpMethodProperties.Contains(methodModifier1))
                CreateParserErrorMessage($"unknown method modifier \"{methodModifier1}\"");
            MethodProperties(out method, methodModifier2, type, name);
            }
        ...
        private void FormalParameter(String ParameterModifierOpt, String type, String name)
            {
            ...
            LastFormalParameterList.Add(new MethodParameterStruct { Implementation = callType, Name = name, Type = type });
            }
        ...
        public MessageTypeOrDestinationEnum CheckUsageOfSymbols(Action<MessageTypeOrDestinationEnum, String> outputMessage)
            {
            var maxMessageType = MessageTypeOrDestinationEnum.noMessageType;
            void OutputMessage(MessageTypeOrDestinationEnum type, String message)
                {
            ...
            if (UsedSymbolsCounter != SymbolDictionary.Count)
                ...
                OutputMessage(MessageTypeOrDestinationEnum.Information,
                   $"Only {UsedSymbolsCounter.ToString()} of {SymbolDictionary.Count.ToString()} symbols are used.");
                }
            else
                {
                OutputMessage(MessageTypeOrDestinationEnum.Information,
                   $"All {SymbolDictionary.Count.ToString()} symbols are used.");
                }
        ...
        private void EvaluateDefinition(Int32 constantPriority, IntMethodClass priorityFunction, VoidMethodClass semanticMethod, bool optimizeTrivialDefinitions)
            ...
            if ((NumberOfElements == 1) && (semanticMethod == null) && (priorityFunction == null) && (constantPriority == 0)
                ...
                Symbol TrivialElement = ActualListOfElements[ActualListOfElements.Count - 1];
                ...
                var NewDefinition =
                    new Definition {
                        ...
                        AttributidentifierArray = ListOfAttributesOfGrammarRule.GetAttributeIdentifiers(NumberOfAttributesOfNewDefinition)
                        };
        ...
        private void EvaluateExcludedTerminalSymbols(BitArray excludedTerminalSymbols)
            ...
            for (int IndexOfTerminalSymbol = 0; IndexOfTerminalSymbol < excludedTerminalSymbols.Length; IndexOfTerminalSymbol++)
                ...
                for (int IndexOfAttribute = 0; IndexOfAttribute < s.AttributenameList.Length; IndexOfAttribute++)
                    {
                    AttributeCounter++;
                    PushAttributeToListOfAttributesOfGrammarRule(s.AttributetypeList[IndexOfAttribute], s.AttributenameList[IndexOfAttribute]);
                    }
        ...
        private void EvaluateMethodParameters(VoidMethodClass semanticMethod)
            ...
            /***** local method *****/
            String AttributeParameterMatchError(AttributeStruct Attribute, MethodParameterStruct Parameter)
                {
                Debug.Assert(Parameter.Name == Attribute.Name);
            ...
            for (Int32 parameterIndex = 0; parameterIndex < MethodParameters.Length; parameterIndex++)
                ...
                Int32 AttributeIndex =
                    ListOfAttributesOfGrammarRule.FindLastIndex
                    (
                        (x) => x.Name == MethodParameters[parameterIndex].Name
                    );
                if (AttributeIndex < 0)
                    {
                    // no attribute with the same name as the formal parameter
                    P1OutputMessageAndLexerPosition(MessageTypeOrDestinationEnum.Error,
                   $"There is no attribute with the same name as the formal parameter \"{MethodParameter.Name}\""
                ...
                if (Attribute.Level < NestingLevel // access to context is not implemented
                    ...
                    P1OutputMessageAndLexerPosition(MessageTypeOrDestinationEnum.Error,
                        $"The attribute \"{MethodParameter.Name}\", "
                ...
                if (Attribute.Type != MethodParameter.Type)
                    {
                    P1OutputMessageAndLexerPosition(MessageTypeOrDestinationEnum.Error,
                   $"The type \"{Attribute.Type}\" of the attribute \"{Attribute.Name}\" differs from the parameters type \"{MethodParameter.Type}\" ");
                    MethodParameter.Implementation = ParameterImplementation.NotAssigned;
                    MethodParameter.Implementation = ParameterImplementation.NotAssigned;
                    continue;
                    }
                ...
                if (Attribute.OverlayType == AttributeStruct.OverlayEnum.inOutAttribute)
                    ...
                    Debug.Assert(OverlayedAttribute.LeftSide);
                    Debug.Assert(OverlayedAttribute.Name == MethodParameter.Name);
                    ...
                    for (Int32 SearchIndex = 0; SearchIndex < MethodParameters.Length; SearchIndex++)
                        {
                        if (MethodParameters[SearchIndex].Name == OverlayedAttribute.Name)
                            {
                    ...
                    if (Attribute.OverlayType != AttributeStruct.OverlayEnum.inOutAttribute
                        ...
                        if (semanticMethod == null)
                            ...
                            P1OutputMessageAndLexerPosition(MessageTypeOrDestinationEnum.Error,
                               ...
                               + $"\"{ListOfAttributesOfGrammarRule[indexOfLeftSideAttrib].Name}\""
                               );
                            }
                        else
                            {
